...
function applyRetrieveZipStatusClasses(li, status) {
  li.classList.remove(
    'retrieve-zip-equal',
    'retrieve-zip-diff',
    'retrieve-zip-left-only',
    'retrieve-zip-right-only'
  );
  if (status === 'match') li.classList.add('retrieve-zip-equal');
  else if (status === 'diff') li.classList.add('retrieve-zip-diff');
  else if (status === 'leftOnly') li.classList.add('retrieve-zip-left-only');
  else if (status === 'rightOnly') li.classList.add('retrieve-zip-right-only');
}
...
function appendMetadataTypeCompareStatusBadge(li, item) {
  ...
  if (!cache?.metadataTypeCompare) return;

  let status =
    cache.pathStatusByRelativePath?.[item.descriptor.relativePath] ||
    (item.descriptor.metadataTypeCompareMember
      ? cache.memberStatusByKey?.[item.descriptor.metadataTypeCompareMember]
      : null);
  if (!status) return;

  applyRetrieveZipStatusClasses(li, status);

  const badge = document.createElement('span');
  badge.className = `retrieve-zip-status-badge retrieve-zip-status-badge--${status}`;
  badge.textContent = t(`metadataTypeCompare.status.${status}`);
  badge.setAttribute('aria-label', t(`metadataTypeCompare.status.${status}`));
...
export function createListItem(item, displayIndex) {
  ...
  appendMetadataTypeCompareStatusBadge(li, item);
